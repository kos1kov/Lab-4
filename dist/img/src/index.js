...
function loadSources(){
    const url = 'https://newsapi.org/v2/sources?apiKey=eeafe868640a463f8550c1c0c88e29f0';
  const request = new Request(url);
  fetch(request)
    .then(function(response) { return response.json(); })
    .then(function(data) {
      for (let i = 0; i < data.sources.length; i++) {
        document.querySelector('#sources').innerHTML += '<button class="btn_sources" id="' + data.sources[i].id + '">' + data.sources[i].name + '</button>';
      }
    });
}
...
function createBlock(newsCount, data){
  ...
  const news_item = document.querySelector('#news-item');
...
function loadBy(urlPart){
  hide('#error-block');
    const url = 'https://newsapi.org/v2/' + urlPart + 'apiKey=eeafe868640a463f8550c1c0c88e29f0';
  const request = new Request(url);
  fetch(request)
    .then(function(response) { return response.json(); })
    .then(function(data) {  
